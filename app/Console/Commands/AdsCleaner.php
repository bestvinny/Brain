...
class AdsCleaner extends Command
{
     ...
     * @var int
     */
    private $unactivated_ads_expiration = 30; // Delete the Inactive ads after this expiration
    private $activated_ads_expiration = 150; // Archive the Active ads after this expiration
			...
			foreach ($ads as $ad)
			{
                ...
                else
                {
                    ...
                    if ($ad->featured == 1)
                    {
                        ...
                        /* It is a website with Premium Job Post Ads */
                        if ($packages->count() > 0) {
                    ...
                    else
                    {
                        // Auto-archive
                        if ($ad->archived != 1) {
                            // Archive all Active ads after '$this->activated_ads_expiration' days
                            if ($today->diffInDays($ad->created_at) >= $this->activated_ads_expiration) {
                                // Archive
                                $ad->archived = 1;
                                $ad->save();
