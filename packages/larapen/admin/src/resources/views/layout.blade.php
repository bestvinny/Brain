      ...
      <footer class="main-footer">
        @if (config('larapen.admin.show_powered_by'))
            <div class="pull-right hidden-xs">
                <a href="https://www.wanekeyasam.co.ke" target="_blank"> {{ trans('admin::messages.powered_by') }}.</a>
            </div>
              {{ t('Copyright') }} &copy; {{ date('Y') }} <a href="{{ url('/') }}" style="padding: 0;"> {{ config('settings.app_name') }}</a> <i class="fa fa-angle-double-right"></i> {{ t('All Rights Reserved') }}.
