								...
								<form class="form-horizontal" id="adForm" method="POST" action="{{ lurl('update/' . $ad->id) }}"
									...
									<fieldset>
										...
										<div class="form-group required <?php echo ($errors->has('description')) ? 'has-error' : ''; ?>">
											<label class="col-md-3 control-label" for="description">{{ t('Job Description') }} <sup>*</sup></label>
                                            <div class="col-md-11" style="position: relative; float: right; padding-top: 10px;">
												...
												<p class="help-block">{{ t('Describe your job posting ad, don\'t forget to include the number of people you wish to hire for the position, the qualifications, skill set required, responsibilities etc.') }}</p>
											</div>
										</div>
										...
										@if (isset($packages) and isset($payment_methods) and $packages->count() > 0 and $payment_methods->count() > 0)
											<div class="well" style="padding-bottom: 0;">
												<h3><i class="icon-certificate icon-color-1"></i> {{ t('Premium Job Post Ad') }} </h3>
												<p>
