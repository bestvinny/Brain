			...
			<div class="col-sm-8 page-content no-margin no-padding">
				@if (isset($cities))
					<div class="relative" style="text-align: center;">
						...
						@if (!auth()->user())
							<a class="btn btn-post btn-darkblue" href="{{ lurl(trans('routes.signup')) . '?type=3' }}" style="padding-left: 30px; padding-right: 30px; text-transform: none;">
								{{ t('Upload Your Resume') }}
							</a>
						@else
							@if (in_array($user->user_type_id, [1, 2]))
								<a class="btn btn-post btn-darkblue" href="{{ lurl(trans('routes.create')) }}" style="padding-left: 30px; padding-right: 30px; text-transform: none;">
									{{ t('Post a Job') }}
								</a>
							@endif
						@endif
