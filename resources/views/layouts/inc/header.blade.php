		...
		<div class="container">
			...
			<div class="navbar-collapse collapse">
				...
				<ul class="nav navbar-nav navbar-right">
					@if (!auth()->user())
						<li><a href="{{ lurl(trans('routes.login')) }}"><i class="fa fa-sign-in"></i> {{ t('Login') }}</a></li>
						<li><a href="{{ lurl(trans('routes.signup')) }}"><i class="icon-user-add fa"></i> {{ t('Signup') }}</a></li>
						<li class="postadd">
							<a class="btn btn-block btn-post btn-darkblue"
					...
					@else
						@if (isset($user))
                            ...
                            @if (in_array($user->user_type_id, [1, 2]))
							<li class="postadd">
								<a class="btn btn-block btn-post btn-darkblue" href="{{ lurl(trans('routes.create')) }}">{{ t('Create a Job ad') }}</a>
							</li>
							@endif
						@endif
					@endif
